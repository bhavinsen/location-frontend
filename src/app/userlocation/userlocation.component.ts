...
export class UserlocationComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.messageForm.value.latitude && this.messageForm.value.longitude !== null) {
      this.apiService.messagePost(this.messageForm.value).subscribe(
        ...
        (err: HttpErrorResponse) => {
          if (err.error) {
            if (err.error.code) {
              this.toastr.error('Oops...', err.error.code)
              this.toastr.error('Oops...', "Please Re-Login User")
            } else {
      ...
      this.toastr.error("Before Form Submit Allow location")
    }
  }
